...
class VnetSurvival(object):
    ...
    def __init__(self,
        ...
        :param time_horizon: Number of neurons on last layer 
...
class VnetSurvival_mixdata(object):
    """
    Implements VNet architecture https://arxiv.org/abs/1606.04797
    """
    def __init__(self,
    ...
    def create_model(self):
        input_shape = tuple(list(self.image_shape) + [self.in_channels])
        input_ = tf.keras.layers.Input(shape=input_shape, dtype=tf.float32, name="input")
        output_ = self.build_network(input_)        
        #output_ = tf.keras.layers.Softmax(name='output')(logits)
        model = tf.keras.models.Model(input_, output_, name='VNetSurvival_cnn')
        return model

def create_mixed_data_network(dim_mlp,
                 image_shape,
                 in_channels,
                 out_channels,
                 time_horizon,
    ...
    mlp = create_mlp(dim_mlp)
    cnn = VnetSurvival_mixdata(image_shape,
            in_channels,
            out_channels,
            time_horizon,
            channels_last,
    ...
    if time_horizon<400:
        x = tf.keras.layers.Dense(512, activation='relu', name='dense_1')(combined_input)
        ...
        x = tf.keras.layers.Dense(self.time_horizon, activation='relu', name='dense_4')(x)
        x = tf.keras.layers.Softmax(name='output')(x)
        model=Model(inputs=[mlp.input, ccn.input], outputs=x,name='VNetSurvival_mixed_data')
        return model
